...
const hostProxies = computed(() => {
  ...
  return Object.values(config.value.proxies)
    ...
    .map(p => ({ id: p.id, name: p.label || p.host || 'Unnamed', icon: 'bi-pc-display', color: p.color }))
})
...
const pacScripts = computed(() => {
  ...
  return Object.values(config.value.pacs)
    .filter(p => p.showInPopup !== false || activeProfileId.value === p.id)
    .map(p => ({ id: p.id, name: p.name || p.url || 'Unnamed', icon: 'bi-file-earmark-code', color: p.color }))
})
...
const autoPolicies = computed(() => {
  ...
  return Object.values(config.value.policies)
    .filter(p => p.showInPopup !== false || activeProfileId.value === p.id)
    .map(p => ({ id: p.id, name: p.name || p.id, icon: 'bi-signpost-split-fill', color: p.color }))
})
...
// Navigation
const openOptions = () => chrome.runtime.openOptionsPage ? chrome.runtime.openOptionsPage() : window.open(chrome.runtime.getURL('options.html'))
...
const showQuickTab = computed(() => {
    ...
    return (policy && (policy.rules || policy.defaultProfileId)) && (failedDomains.value.length > 0 || !!contextMenuDomain.value)
})
...
const failedDomains = computed(() => {
    ...
    return Object.keys(groups).map(root => (groups[root].length > 1 || domains.includes(root)) ? `.${root}` : groups[root][0]).sort()
})

watch(failedDomains, (newVal) => { selectedDomains.value = [...newVal] }, { immediate: true })

const proxyOptions = computed(() => {
  if (!config.value) return []
  const groups = []
  if (config.value.proxies) {
      const p = Object.values(config.value.proxies).map(p => ({ id: p.id, label: p.label || p.name })).sort((a, b) => a.label.localeCompare(b.label))
      if (p.length > 0) groups.push({ label: t('lblProxies'), options: p })
  }
  if (config.value.proxyGroups) {
      const g = Object.values(config.value.proxyGroups).map(g => ({ id: g.id, label: g.name })).sort((a, b) => a.label.localeCompare(b.label))
...
const confirmQuickAdd = async () => {
    ...
    const newRules = selectedDomains.value.map(pattern => ({
        ...
        type: 'rule', ruleType: 'wildcard', pattern, proxyId: quickProxyId.value, valid: true
    }))
