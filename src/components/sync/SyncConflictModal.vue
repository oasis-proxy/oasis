<template>
  ...
  >
            ...
            <div class="d-flex align-items-center gap-3 mb-4 position-relative z-10">
              <div>
                <div class="d-flex align-items-center gap-2">
                  <h3 class="text-sm fw-semibold ui-text-primary m-0">
                    {{ $t('lblLocalVersion') }}
                  </h3>
                </div>
    ...
    <template #footer>
      <button
        type="button"
        class="btn ui-button-secondary h-8 px-3 text-xs fw-medium rounded-lg d-flex align-items-center"
        @click="emit('cancel')"
      >
        {{ $t('btnCancel') }}
      </button>
      <button
        type="button"
        class="btn ui-button-danger h-8 px-3 text-xs fw-medium rounded-lg d-flex align-items-center"
...
const configVersion = computed(() =>
  props.localConfig.version ? `v${props.localConfig.version}` : 'v1'
)
...
// Cloud Computed
const cloudConfigVersion = computed(() =>
  props.cloudConfig?.version ? `v${props.cloudConfig.version}` : '-'
)
