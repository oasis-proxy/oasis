<template>
  <BaseModal :visible="visible" :title="$t('rscmTitle')" maxWidth="720px" @close="emit('close')">
    <div class="d-flex flex-column gap-4 h-100" style="min-height: 480px">
      ...
      <div v-if="url">
        <label class="d-flex flex-column gap-2 w-100 mb-0">
          ...
          <div
            :title="url"
            class="d-flex align-items-center px-3 py-2 rounded-lg border ui-input text-xs ui-text-secondary text-truncate font-monospace"
      ...
      <div v-if="lastUpdated">
        <label class="d-flex flex-column gap-2 w-100 mb-0">
          <span class="ui-text-primary text-xs fw-medium lh-1">{{
            $t('rscmLabelLastUpdated')
          }}</span>
          <div
            class="d-flex align-items-center px-3 py-2 rounded-lg border ui-input text-xs ui-text-secondary"
            style="min-height: 28px"
          >
            {{ formattedTime }}
          </div>
