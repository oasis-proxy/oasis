  ...
  <BaseModal :visible="visible" :title="$t('smmmTitle')" maxWidth="800px" @close="emit('close')">
    <div class="d-flex flex-column gap-4 h-100" style="max-height: 70vh;">
      ...
      <section>
        ...
        <select v-model="targetPolicyId" :disabled="!!forcedTargetId" :class="{'bg-subtle ui-text-secondary cursor-not-allowed': !!forcedTargetId}" class="form-select ui-input w-100 rounded-lg border px-3 text-xs shadow-sm">
        ...
        <div class="rounded-lg border border-subtle overflow-hidden shadow-sm">
          ...
          <div class="overflow-y-auto custom-scrollbar ui-bg-card" style="max-height: 12rem;">
            <div v-for="(rule, idx) in sourceRules" :key="idx" class="d-flex align-items-center gap-1 px-2 py-2 opacity-70">
              <div style="width: 30%;"><input type="text" class="form-control ui-input ui-input-sm w-100 rounded border py-0 px-2 bg-subtle" :value="$t('opt' + (rule.ruleType.charAt(0).toUpperCase() + rule.ruleType.slice(1)))" readonly /></div>
              <div style="width: 50%;"><input type="text" class="form-control ui-input ui-input-sm w-100 rounded border py-0 px-2 font-monospace bg-subtle" :value="rule.pattern" readonly /></div>
              <div style="width: 20%;"><input type="text" class="form-control ui-input ui-input-sm w-100 rounded border py-0 px-2 text-muted bg-subtle" :value="getProxyLabel(rule.proxyId)" readonly /></div>
            </div>
    ...
    <template #footer>
      <button @click="emit('close')" class="px-3 py-2 rounded-lg text-xs fw-medium ui-button-secondary transition-colors">{{ $t('btnCancel') }}</button>
      <button @click="handleConfirm" :disabled="!isValid" class="px-3 py-2 rounded-lg text-xs fw-bold ui-button-primary shadow-lg transition-colors disabled:opacity-50">{{ $t('smmmBtnMerge', [mergedRules.length]) }}</button>
    </template>
  </BaseModal>
</template>
...
const {
  targetPolicyId, conflictMode, mergedRules, proxyList, availablePolicies, isValid,
  handleConfirm, removeMergedRule
