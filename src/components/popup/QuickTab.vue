    ...
    <div v-else class="flex-1 d-flex flex-column min-h-0">
      ...
      <div class="d-flex gap-3 pb-2 px-2">
          <div class="flex-1 d-flex flex-column gap-1">
            ...
            <select :value="proxyId" @change="$emit('update:proxyId', $event.target.value)" class="form-select ui-input ui-input-sm w-100 text-xs cursor-pointer">
          ...
          <div class="flex-1 d-flex flex-column gap-1">
            ...
            <select :value="destination" @change="$emit('update:destination', $event.target.value)" class="form-select ui-input ui-input-sm w-100 text-xs cursor-pointer">
      ...
      <div class="quick-section justify-content-end gap-2 pb-3"> 
        <button @click="$emit('cancel')" class="px-3 py-2 text-xs font-medium ui-button-secondary rounded-lg transition-all">
          {{ $t('btnCancel') }}
        </button>
        <button @click="$emit('confirm')" :disabled="modelValue.length === 0" class="px-3 py-2 text-xs font-medium ui-button-primary rounded-lg shadow-primary-lg transition-colors border-0">
          {{ $t('btnConfirm') }}
        </button>
      </div>
    </div>
...
const emit = defineEmits(['update:modelValue', 'update:proxyId', 'update:destination', 'cancel', 'confirm'])
