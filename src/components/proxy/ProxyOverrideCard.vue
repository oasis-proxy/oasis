<template>
  <div class="ui-card rounded-xl border border-subtle shadow-sm overflow-hidden">
        ...
        <div class="col-2">
          <label class="ui-form-group">
            ...
            <input
              ...
              @input="
                updateField('port', $event.target.value ? parseInt($event.target.value) : null)
              "
              @blur="validatePort"
              type="number"
              min="1"
    ...
    >
      <h4
        class="text-sm fw-medium ui-text-primary mb-3 ls-wide d-flex align-items-center justify-content-between"
      >
        {{ $t('phHeaderAuth') }}
        <span class="text-xs ui-text-secondary font-normal normal-case">{{
          $t('phDescAuth')
        }}</span>
      </h4>
      <div class="row g-3">
