<template>
  <BaseModal :visible="visible" :title="$t('pcmTitleClone')" @close="emit('close')">
    <div class="d-flex flex-column gap-3">
      <!-- Source Display -->
      <label class="d-flex flex-column gap-2 w-100">
        <span class="ui-text-primary text-xs fw-medium lh-1">{{ $t('pcmLabelSource') }}</span>
        <div
          class="ui-input px-3 rounded-lg border text-xs ui-text-secondary d-flex align-items-center gap-2 overflow-hidden"
        >
