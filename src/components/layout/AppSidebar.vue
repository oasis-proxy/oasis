<template>
  <aside
    class="ui-bg-card border-end border-subtle d-flex flex-column shrink-0 transition-colors"
    style="width: 18rem"
  >
    <SidebarBrand />
    ...
    <nav class="flex-1 overflow-y-auto custom-scrollbar p-3 d-flex flex-column gap-4">
      ...
      <nav-group :title="$t('navPolicyRules')" @add="showPolicyModal = true">
        <SidebarItem
          v-for="r in policyRules"
          :key="r.id"
          :to="r.type === 'pac' ? `/pac/${r.id}` : `/policy/${r.id}`"
          :icon="r.icon"
          :label="r.name"
          :color="r.color"
        />
      ...
      h('div', [
        h('div', { class: 'mb-2 d-flex align-items-center justify-content-between group' }, [
          h(
            'h3',
            {
              class: 'text-xs fw-semibold ui-text-secondary text-uppercase m-0',
          ...
          slots.add
            ? slots.add()
            : attrs.onAdd
              ? h('button', { class: 'ui-button-icon', onClick: () => emit('add') }, [
                  h('i', { class: 'bi bi-plus-lg text-sm' })
                ])
              : null
        ]),
