<template>
  ...
  >
    ...
    <div style="width: 8%;" class="d-flex align-items-center justify-content-around">
      <button @click="$emit('add-rule-below')" class="ui-button-icon" :title="$t('btnAddRuleBelow')">
        <i class="bi bi-plus-lg text-xs"></i>
      </button>
      <button @click="$emit('add-divider-below')" class="ui-button-icon" :title="$t('btnAddDividerBelow')">
...
defineEmits(['dragstart', 'dragover', 'drop', 'dragend', 'edit', 'update:editingLabel', 'save', 'cancel', 'add-rule-below', 'add-divider-below', 'delete'])
