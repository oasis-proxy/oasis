<template>
  <BaseDetailView v-if="proxy" :title="proxy.label || proxy.host || 'Unnamed Proxy'">
    ...
    <template #actions>
      ...
      >
        <input
          ...
          role="switch"
          id="showInPopup"
          v-model="proxy.showInPopup"
        />
        <label
          class="form-check-label text-xs font-medium ui-text-secondary"
          style="cursor: pointer"
          for="showInPopup"
    ...
    <section>
      ...
      <div class="ui-card rounded-xl border shadow-sm overflow-hidden">
        ...
        <div class="px-4 pt-3 pb-4 border-top border-light">
          <h4
            class="text-sm font-medium ui-text-primary mb-4 d-flex align-items-center justify-content-between"
          >
            {{ $t('phHeaderAuth') }}
    ...
    <section>
      <div class="ui-card-label">
        <span class="label-text">{{ $t('phHeaderAdvanced') }}</span>
      </div>
