  ...
  <BaseDetailView :title="policy.name || $t('lblAutoPolicy')" maxWidth="6xl">
    ...
    <section>
      <div class="ui-card-label">
        ...
        <div class="d-flex align-items-center gap-2">
          <button @click="openSmartMerge" class="ui-button-icon sm" :title="$t('smmmTitle')">
    ...
    <section class="mt-4">
      <div class="ui-card-label">
        ...
        <div class="d-flex align-items-center gap-2">
          <button @click="openRejectSmartMerge" class="ui-button-icon sm" :title="$t('smmmTitle')">
            <i class="bi bi-diagram-3 ui-icon-md"></i>
          </button>
          <button
            @click="showRejectAutoProxyPreview = true"
            class="ui-button-icon sm"
            :title="$t('appmBtnPreview')"
          >
            <i class="bi bi-filetype-txt ui-icon-md"></i>
    ...
    <SmartRulesMergeModal
      ...
      :proxies="config?.proxies"
      :proxyGroups="config?.proxyGroups"
      :forcedTargetId="policy.id"
      :domainOptimize="true"
      @close="showSmartMerge = false"
      @merge="handleSmartMerge"
    />
    <AutoProxyPreviewModal
      :visible="showRejectAutoProxyPreview"
      :rules="policy.rejectRules"
      :proxies="config?.proxies"
      :proxyGroups="config?.proxyGroups"
      :hideProxyFilter="true"
      @close="showRejectAutoProxyPreview = false"
    />
    <SmartRulesMergeModal
      :visible="showRejectSmartMerge"
      :policies="config?.policies || {}"
      :sourceRules="rejectSmartMergeSourceRules"
      :proxies="config?.proxies"
      :proxyGroups="config?.proxyGroups"
      :forcedTargetId="policy.id"
      :domainOptimize="true"
      lockedProxy="reject"
      :hideConflict="true"
      @close="showRejectSmartMerge = false"
...
const {
  validationErrors: rejectValidationErrors,
  validateRule: validateRejectRule,
  revalidateAll: revalidateAllRejectRules,
  addRule: addRejectRule,
  insertRuleBelow: insertRejectRuleBelow,
  insertDividerBelow: insertRejectDividerBelow,
  deleteRule: deleteRejectRule,
...
const startEditRejectDivider = (index, label) => {
  editingRejectDividerIndex.value = index
  editingRejectDividerLabel.value = label || t('lblNewSection')
}
...
const handleSmartMerge = ({ conflictMode, rules: mergedRules }) => {
  ...
  mergedRules.forEach((mr) => {
    const existingIndex = policy.value.rules.findIndex(
...
const handleRejectSmartMerge = ({ conflictMode, rules: mergedRules }) => {
  ...
  const indicesToRemove = new Set()
  mergedRules.forEach((mr) => {
    const existingIndex = policy.value.rejectRules.findIndex(
      (r) => r.type !== 'divider' && r.ruleType === mr.ruleType && r.pattern === mr.pattern
    )
    if (existingIndex !== -1) {
      if (conflictMode === 'overwrite') {
