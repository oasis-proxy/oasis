<template>
  <BaseDetailView :title="pac.name || pac.url || $t('unnamedPAC')">
      ...
      <div class="ui-card rounded-xl border border-subtle ui-bg-card shadow-sm overflow-hidden">
        ...
        <div class="px-4 pt-4 pb-4 d-flex flex-column gap-4">
          ...
          <div class="ui-form-group">
            <span class="ui-text-primary text-xs fw-medium lh-1">{{
              $t('pacLabelSourceMethod')
            }}</span>
            <div class="d-flex align-items-center gap-4">
              <label class="d-flex align-items-center gap-2" style="cursor: pointer">
                ...
                />
                <span class="text-xs ui-text-primary transition-colors">{{
                  $t('pacLabelRemoteUrl')
                }}</span>
              </label>
              <label class="d-flex align-items-center gap-2" style="cursor: pointer">
          ...
          <div v-if="pac.mode === 'remote'" class="row g-3">
            ...
            <div class="col-4 d-flex flex-column justify-content-end align-items-start">
              <button
                @click="fetchPacContent"
                class="px-3 py-1 rounded-lg text-xs fw-medium ui-button-secondary transition-colors d-flex align-items-center justify-content-center gap-2"
              >
    ...
    <section class="d-flex flex-column" style="height: 600px">
      ...
      <div
        class="ui-card rounded-xl border shadow-sm overflow-hidden d-flex flex-column h-100 w-100 flex-1"
      >
        <div class="ui-card-header justify-content-end">
          <span
            v-if="pac.mode === 'remote'"
            class="px-2 py-1 rounded bg-subtle text-xs font-monospace ui-text-secondary"
            >{{ $t('pacMsgReadOnly') }}</span
          >
          <span
            v-else
            class="px-2 py-1 rounded bg-subtle text-xs font-monospace ui-text-secondary"
            >{{ $t('pacMsgEditable') }}</span
          >
        </div>
