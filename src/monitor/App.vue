<template>
  <div class="d-flex flex-column h-100">
    ...
    <main class="flex-1 d-flex overflow-hidden">
      ...
      >
        <div
          class="d-flex align-items-center justify-content-between px-4 py-2 border-bottom"
        ...
        >
          <div class="d-flex align-items-center gap-3">
            ...
            <span class="text-xs ui-text-tertiary"
              >{{ filteredRequests.length }} {{ $t('lblRequestsSuffix') }}</span
            >
          </div>
        ...
        <MonitorRequestTable :requests="filteredRequests" :ipTags="ipTags" @copy="copyText" />

        <div
          class="d-flex align-items-center justify-content-between px-4 py-2 border-top text-xs ui-text-tertiary"
          style="background: var(--ui-bg-subtle)"
        >
          <span
            >{{ $t('lblShowing') }} {{ filteredRequests.length }} {{ $t('lblOf') }}
            {{ totalRequestCount }} {{ $t('lblRequestsSuffix') }}</span
          >
...
const { isDark, applyTheme, darkIconUrl, lightIconUrl, darkBrandUrl, lightBrandUrl } =
  useMonitorTheme()
const { ipTags, loadMatcherConfig, matchRule } = useMonitorMatcher()
const { requests, tabs, handleMessage, clearRequests, getHostname } = useMonitorData(
  historyLimit,
  matchRule
)
...
const filteredRequests = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(
      (r) => r.url.toLowerCase().includes(query) || r.method.toLowerCase().includes(query)
    )
  }
...
const getSelectedTabTitle = () =>
  selectedTabId.value && tabs.value[selectedTabId.value]
    ? tabs.value[selectedTabId.value].title || 'Untitled'
    : 'All Requests'
const getTabRequestCount = (tabId) => requests.value.filter((r) => r.tabId === tabId).length
const clearCurrentRequests = () => {
  clearRequests(selectedTabId.value)
  if (selectedTabId.value !== null) selectedTabId.value = null
}
...
const openQuickAdd = async () => {
  ...
  quickAddSourceRules.value = Array.from(domains)
    .sort()
    .map((domain) => ({ ruleType: 'wildcard', pattern: domain, proxyId: 'direct' }))
...
const handleQuickAddMerge = async ({ targetId, conflictMode, rules: mergedRules }) => {
  ...
  mergedRules.forEach((rule) => {
    const existingIndex = targetPolicy.rules.findIndex(
      (r) => r.type !== 'divider' && r.ruleType === rule.ruleType && r.pattern === rule.pattern
    )
    if (existingIndex !== -1) {
...
onMounted(async () => {
  ...
  const themeListener = () =>
    loadConfig().then((c) => {
      if (c.ui?.theme === 'auto') applyTheme('auto')
    })
