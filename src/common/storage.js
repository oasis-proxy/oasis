...
export { loadConfig, saveGeneralSettings, touchConfig } from './storage/core'
// Re-export sync functions
export { syncToCloud, syncFromCloud, clearCloudConfig, hydrateConfig, optimizePayload } from './storage/sync'
...
export async function saveConfig(config, skipSync = false, skipTouch = false) {
  await core.saveConfig(config, skipSync, skipTouch)
  if (!skipSync) {
      await sync.triggerAutoSync(config)
  }
}
...
export async function saveProxies(proxies, skipSync = false) {
  const fullConfig = await core.loadConfig()
  fullConfig.proxies = JSON.parse(JSON.stringify(proxies || {}))
  core.touchConfig(fullConfig)
...
export async function savePolicies(policies, skipSync = false, skipTouch = false) {
  ...
  if (!skipTouch) core.touchConfig(fullConfig)
  await saveConfig(fullConfig, skipSync)
}
...
export async function importConfig(jsonString) {
    try {
        ...
        await sync.hydrateConfig(newConfig)
