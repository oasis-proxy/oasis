 ...
 */
export function generatePacScriptFromPolicy(
  policy,
  proxies,
  rejectConfig = null,
  tempRules = [],
  ...
  const getSingleProxyString = (proxyId, protocol = null) => {
    ...
    if (
      protocol &&
      proxy.overrides &&
      proxy.overrides[protocol] &&
      proxy.overrides[protocol].scheme !== 'default'
    ) {
  ...
  const formatProxyString = (scheme, host, port) => {
    ...
    // HTTP/HTTPS → PROXY (PAC standard, supports both HTTP and HTTPS traffic)
