...
export function useRuleMerge(props, emit) {
    ...
    const getRegisteredDomain = (hostname) => {
        ...
        const twoPartTlds = new Set(['co.uk', 'org.uk', 'ac.uk', 'gov.uk', 'com.au', 'net.au', 'org.au', 'com.cn', 'net.cn', 'org.cn', 'co.jp', 'or.jp', 'ne.jp', 'co.kr', 'or.kr', 'com.br', 'org.br', 'co.nz', 'org.nz', 'com.tw', 'org.tw', 'com.hk', 'org.hk', 'com.sg', 'org.sg', 'co.in', 'org.in', 'net.in', 'com.mx', 'org.mx', 'co.za', 'org.za'])
    ...
    const handleConfirm = () => {
        if (isValid.value) {
            emit('merge', { targetId: targetPolicyId.value, conflictMode: conflictMode.value, rules: mergedRules.value })
            emit('close')
        }
    }
    ...
    watch(() => props.visible, (newVal) => { if (newVal) reset() })

    return {
        targetPolicyId, conflictMode, mergedRules, proxyList, availablePolicies, isValid,
        handleConfirm, removeMergedRule: (index) => mergedRules.value.splice(index, 1)
    }
}
